...
const sidebarDOM = () => {
    ...
    if (verticalLayout) {
      expand
        ? (nav.classList.remove("nav-collapse"),
          nav.classList.add("nav-collapse-vertical"),
          expColIcon.classList.add("exp-col-flip"))
        : (nav.classList.remove("nav-collapse-vertical"),
          nav.classList.remove("nav-collapse"),
          expColIcon.classList.remove("exp-col-flip"));
    } else {
      expand
        ? (nav.classList.remove("nav-collapse-vertical"),
    ...
    if (bool) {
      setTimeout(() => {
        nav.style.transition =
          "max-height .5s ease-in-out, padding-bottom .5s linear, padding-top .5s linear";
      }, 500);
      ...
      setTimeout(() => {
        nav.style.transition =
          "max-width .5s ease-in-out, padding-left .5s linear, padding-right .5s linear";
      }, 500);
  ...
  document.getElementById("list-name").addEventListener("keypress", () => {
    document.getElementById("list-name").setCustomValidity("");
  });

  const newListFunc = () => {
    if (poo.includes(document.getElementById("list-name").value)) {
      document
        .getElementById("list-name")
        .setCustomValidity("You cannot have two lists with the same name!");
    } else {
      document.getElementById("list-name").setCustomValidity("");
    }
